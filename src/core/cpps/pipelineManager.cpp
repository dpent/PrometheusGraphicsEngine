...
void PipelineManager::createParticleComputePipeline(ParticleEffect& pEffect) {

    auto compShaderCode = Engine::readFile((std::filesystem::path(BIN_DIR) / pEffect.shaderFilename).string());
    ...
    if (vkCreatePipelineLayout(Engine::deviceInfo.logicalDevice, &pipelineLayoutInfo, nullptr, &pEffect.pipeline.layout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create compute pipeline layout!");
    }

    VkComputePipelineCreateInfo pipelineInfo{};
    pipelineInfo.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
    pipelineInfo.layout = pEffect.pipeline.layout;
    pipelineInfo.stage = compShaderStage;

    if (vkCreateComputePipelines(Engine::deviceInfo.logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pEffect.pipeline.pipeline) != VK_SUCCESS) {
...
void PipelineManager::createParticleGraphicsPipeline() {
    auto vertShaderCode = Engine::readFile((std::filesystem::path(BIN_DIR) / "particlesVert.spv").string());
    auto fragShaderCode = Engine::readFile((std::filesystem::path(BIN_DIR) / "particlesFrag.spv").string());
    ...
    if (vkCreatePipelineLayout(Engine::deviceInfo.logicalDevice, &pipelineLayoutInfo, nullptr, &Engine::particlePipeline.layout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create pipeline layout!");
    }
    ...
    pipelineInfo.pColorBlendState = &colorBlending;
    pipelineInfo.pDynamicState = &dynamicState;
    pipelineInfo.layout = Engine::particlePipeline.layout;
    pipelineInfo.renderPass = Engine::graphicsRenderPass;
    pipelineInfo.subpass = 0;
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
    pipelineInfo.basePipelineIndex = -1; // Optional
    ...
    if (vkCreateGraphicsPipelines(Engine::deviceInfo.logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &Engine::particlePipeline.pipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create graphics pipeline!");
    }
