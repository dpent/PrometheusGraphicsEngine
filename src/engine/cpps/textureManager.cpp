...
namespace Prometheus{
    uint32_t TextureManager::createTextureImage(std::string filename, int req_comp, VkDevice& device, VkPhysicalDevice& physicalDevice, 
        ...
        TextureManager::createImage(texWidth, 
            texHeight, 
    ...
    void TextureManager::createImage(uint32_t width, uint32_t height, VkFormat format, 
        ...
        allocInfo.memoryTypeIndex = BufferManager::findMemoryType(memRequirements.memoryTypeBits, properties,physicalDevice);
    ...
    void TextureManager::createShadowMapSampler(VkDevice& device, VkSampler& sampler) {
        VkSamplerCreateInfo samplerInfo{};
        samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
        samplerInfo.magFilter = VK_FILTER_LINEAR;
        samplerInfo.minFilter = VK_FILTER_LINEAR;
