...
namespace Prometheus{
    ...
    void GraphicsPipelineManager::createEditorPreGraphicsPipeline(VkDevice& device){
        ...
        */
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &Engine::preGraphicsLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }
        ...
        pipelineInfo.pColorBlendState = &colorBlending;
        pipelineInfo.pDynamicState = &dynamicState;
        pipelineInfo.layout = Engine::preGraphicsLayout;
        pipelineInfo.renderPass = Engine::renderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
        pipelineInfo.basePipelineIndex = -1; // Optional
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &Engine::preGraphicsPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
    ...
    void GraphicsPipelineManager::createDebugPipeline(VkDevice& device){
        ...
        auto bindingDescription = Vertex::getDebugBindingDescription();
        auto attributeDescriptions = Vertex::getDebugAttributeDescriptions();
        ...
        */
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &Engine::debugPipelineLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }
        ...
        pipelineInfo.pColorBlendState = &colorBlending;
        pipelineInfo.pDynamicState = &dynamicState;
        pipelineInfo.layout = Engine::debugPipelineLayout;
        pipelineInfo.renderPass = Engine::renderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
        pipelineInfo.basePipelineIndex = -1; // Optional
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &Engine::debugPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
    ...
    void GraphicsPipelineManager::createLightBillBoardPipeline(VkDevice& device){
        ...
        VkPipelineShaderStageCreateInfo shaderStages[] = {
            createShaderStageInfo(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
                                VK_SHADER_STAGE_VERTEX_BIT, vertShaderModule, "main", nullptr),
            createShaderStageInfo(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
                                VK_SHADER_STAGE_FRAGMENT_BIT, fragShaderModule, "main", nullptr)
        };

        // NO vertex input - all procedural!
        VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
        vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        vertexInputInfo.vertexBindingDescriptionCount = 0;
        vertexInputInfo.vertexAttributeDescriptionCount = 0;

        // Triangle strip for efficiency
        VkPipelineInputAssemblyStateCreateInfo inputAssembly{};
        inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
        inputAssembly.primitiveRestartEnable = VK_FALSE;

        // Reuse your dynamic viewport/scissor
        std::vector<VkDynamicState> dynamicStates = {
        ...
        colorBlendAttachment.blendEnable = VK_TRUE;
        colorBlendAttachment.srcColorBlendFactor = VK_BLEND_FACTOR_ONE; // Additive!
        ...
        // Depth test ON, depth write OFF (don't block objects behind)
        VkPipelineDepthStencilStateCreateInfo depthStencil{};
        depthStencil.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
        depthStencil.depthTestEnable = VK_TRUE;
        depthStencil.depthWriteEnable = VK_FALSE;
        ...
        // Can reuse same descriptor set layout and push constants
        VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
        pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pipelineLayoutInfo.setLayoutCount = 1;
        pipelineLayoutInfo.pSetLayouts = &Engine::descriptorSetLayout;
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &Engine::lightPipelineLayout) != VK_SUCCESS) {
        ...
        pipelineInfo.pDynamicState = &dynamicState;
        pipelineInfo.pDepthStencilState = &depthStencil;
        pipelineInfo.layout = Engine::lightPipelineLayout;
        pipelineInfo.renderPass = Engine::renderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &Engine::lightBillboardPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create light billboard pipeline!");
        }
    ...
    void GraphicsPipelineManager::createParticleGraphicsPipeline(VkDevice& device){
        ...
        auto bindingDescription = Particle::getBindingDescriptions();
        ...
        */
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &Engine::particlePipelineLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }
        ...
        pipelineInfo.pColorBlendState = &colorBlending;
        pipelineInfo.pDynamicState = &dynamicState;
        pipelineInfo.layout = Engine::particlePipelineLayout;
        pipelineInfo.renderPass = Engine::renderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
        pipelineInfo.basePipelineIndex = -1; // Optional
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &Engine::particleGraphicsPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
    ...
    void GraphicsPipelineManager::createShadowMapPipeline(VkDevice& device) {

        std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages{};

        auto vertShaderCode = Engine::readFile("./build/shadowMapVert.spv");
        VkShaderModule vertShaderModule = GraphicsPipelineManager::createShaderModule(vertShaderCode, device);

        shaderStages[0] = createShaderStageInfo( //Vertex
            VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
            VK_SHADER_STAGE_VERTEX_BIT,
            vertShaderModule,
            "main",
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &Engine::shadowMapPipelineLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }

        VkGraphicsPipelineCreateInfo pipelineCreateInfo{};
        pipelineCreateInfo.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
        pipelineCreateInfo.layout = Engine::shadowMapPipelineLayout;
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &Engine::shadowMapPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
